...
const Page = () => {
  ...
  return (
    <section className="w-full min-h-screen flex flex-col items-center justify-center">
      ...
      <form onSubmit={(e) => handleLogin(e, setLoading, router)} className="shadow-sm p-6 rounded-xl bg-white box space-y-4 w-full max-w-sm mt-6">
        <div>
          <p className="leading-7 text-[#00000041]">Preencha para acessar</p>
          <h2 className="scroll-m-20 pb-2 text-3xl font-bold tracking-tight ">Portal de Reservas</h2>
        </div>

        <div className="flex flex-col space-y-2">
          <label htmlFor="email" className="text-sm font-medium">Email</label>
          <Input id="email" type="email" name="email" placeholder="seuemail@kroton.com.br" />
        </div>
      ...
      <div className="flex flex-col items-center mt-5">
        <span className="font-semibold cursor-pointer">Esqueci minha senha</span>
        <span>Não tem uma conta? <Link prefetch href="/register" className="font-semibold cursor-pointer">Criar sua conta</Link></span>
      </div>
    </section>
  )
}
